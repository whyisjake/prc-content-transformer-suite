 ...
 * - prcMarkdownForAgents.callback: callable string (e.g. Namespace\\Class::method)
 * - prcMarkdownForAgents.mode: html-fallback|strip|children-only
 *
 * @package PRC\Platform\Markdown_For_Agents
 */
class Block_Markdown_Resolver {
	...
	public static function inject_metadata_into_supports( array $metadata ): array {
		if ( empty( $metadata['prcMarkdownForAgents'] ) || ! is_array( $metadata['prcMarkdownForAgents'] ) ) {
			return $metadata;
		}
		...
		$metadata['supports']['prcMarkdownForAgents'] = $metadata['prcMarkdownForAgents'];
	...
	public static function get_block_config( string $block_name ): ?array {
		...
		$config = $block_type->supports['prcMarkdownForAgents'] ?? null;
		if ( ! is_array( $config ) ) {
			return null;
		}
